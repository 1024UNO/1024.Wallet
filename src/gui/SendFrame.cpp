...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("1024://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "1024:");
    }
    if(!_request.startsWith("1024:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with 1024:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "1HkKSVnbg1GWDQ1w5XCVrGczfwWR1YJ6xQKmf3ZYGMHcLW7mFHRv9e3emtvuJhaMpMeB7iniQZjCKT8NSaBRWbX4VgC7rxJ";
